...
//     const image = product.productResources?.find((r) => r.isPrimary)?.url || '';
...
//       productVariantId: variant?.id, // Ensure the variant ID is included
//       name: product.name,
//       price: product.price,
...
//       size: selectedSize,
//       quantity: 1,
//       stockQuantity: variant?.stockQuantity, // Keep track of stock
//     };
...
const ProductDetailPage = () => {
  ...
  const [selectedSize, setSelectedSize] = useState('');
  const [variantStock, setVariantStock] = useState(null);
  const [activeImage, setActiveImage] = useState(''); // Track the clicked image
  ...
  const handleImageClick = (imageUrl) => {
    setActiveImage(imageUrl); // Set the clicked image as active image to display in the larger image area
  };
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      <div className="flex flex-col md:flex-row gap-10">
        ...
        <div className="md:w-1/2">
          <div className="w-full mb-4">
            {/* Main Image: Clicking it will change the active image */}
            <img
              src={
                activeImage ||
                product.productResources?.find((r) => r.isPrimary)?.url ||
          ...
          <div className="flex gap-4">
            {product.productResources?.map((img) => (
              <img
                ...
                onClick={() => handleImageClick(img.url)} // Set clicked image as active
              />
            ))}
          </div>
        </div>
